...
def main(args):
    ...
    ds_dict = np.load('data/example_tissuenet/dataset_dicts_cell_test.npy', allow_pickle=True)
    rst = []
    k = 0
    for d in tqdm(ds_dict):
        ...
        if k < 2:
            ...
            cv2.imwrite(os.path.join('output/tissuenet', '{}_pred.png'.format(d['image_id'])), out.get_image()[:, :, ::-1])
            k += 1
    ...
    rst = np.array(rst)
    np.save('output/tissuenet/test.npy', rst)
